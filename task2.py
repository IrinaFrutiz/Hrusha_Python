...
def check_regular_bracket_sequence(sequence):
    ...
    index = 0
    print("Ваша скобочная последовательность НЕ является правильной" if stack
          else "Ваша скобочная последовательность Правильная")
    for i in stack:
...
print("Обрабатываемые символы: {, }, [, ], (, ), <, >.",
      "При наличии в последовательности необрабатываемых символов - она не будет считаться правильной.",
      "Непарные и необрабатываемые символы в последовательности будут отмечены красным.",
      "Пример из задания: [((())()(())]].", sep="\n")
